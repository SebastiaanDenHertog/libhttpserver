...
#include "httpserver/details/modded_request.h"
...
// Mingw doesn't implement SIGPIPE
    ...
    // WEBSERVER
    webserver::webserver(const create_webserver &params) : port(params._port),
    ...
    void error_log(void *cls, const char *fmt, va_list ap)
    {
        ...
        if (dws->log_error != nullptr)
            dws->log_error(msg);
    }

    void access_log(webserver *dws, string uri)
    {
        if (dws->log_access != nullptr)
            dws->log_access(uri);
    }
    ...
    MHD_Result webserver::finalize_answer(MHD_Connection *connection, struct details::modded_request *mr, const char *method)
    {
        ...
        { // catches errors in internal error page
